...
class Invaders(GameApp):
    ...
    #
    # Attribute _lastkey: determines if key 's' was pressed last frame
    # Invariant: _lastkey is a boolean that is True if key 's' was pressed last
    # frame and False if otherwise
    #
    ...
    def start(self):
        ...
        if self._state == STATE_INACTIVE:
            self._text = GLabel(text="Press 'S' to Play",font_size=24, left=5, bottom=5)
        else:
            self._text = None
    ...
    def update(self,dt):
        ...
        """
        # IMPLEMENT ME
        self._progress_state_inactive()
        if self._state == STATE_NEWWAVE:
            self._wave = Wave()
            self._state = STATE_ACTIVE
        if self._state == STATE_ACTIVE:
            self._wave.update(dt,self.input)
            if self._wave._ship is None:
                self._state = STATE_PAUSED
                self._wave._lives -= 1
                self._text = GLabel(text="Press 'S' to Resume",font_size=24, left=5, bottom=5)
            if self._wave._aliensgone or self._wave._belowline:
                self._state = STATE_PAUSED
        if self._state == STATE_CONTINUE:
            self._state = STATE_ACTIVE
        self._progress_state_pause()
        if self._state == STATE_PAUSED and (self._wave._lives == 0 or self._wave._belowline):
            self._state = STATE_COMPLETE
            self._text = GLabel(text="You Lose",font_size=50, left=300, bottom=350)
        if self._state == STATE_PAUSED and self._wave._aliensgone:
            self._state = STATE_COMPLETE
            self._text = GLabel(text="You Win",font_size=50, left=300, bottom=350)
    ...
    def _progress_state_inactive(self):
        """
        Determines if the state STATE_INACTIVE should progress to STATE_NEWWAVE

        This method checks for the key press for 's', which occurs when 's' is
        pressed for the first time. When this happens, self._state is changed
        to STATE_NEWWAVE, and self._text becomes None.
        """
        if self._state == STATE_INACTIVE:
            if self.input.is_key_down('s') and not self._lastkey:
                self._state = STATE_NEWWAVE
                self._text = None
            if self.input.is_key_down('s'):
                self._lastkey = True

    def _progress_state_pause(self):
        if self._state == STATE_PAUSED:
            if self.input.is_key_down('s') and not self._lastkey:
                self._state = STATE_ACTIVE
                self._text = None
                self._wave._ship = Ship(GAME_WIDTH/2, SHIP_BOTTOM, SHIP_WIDTH, SHIP_HEIGHT)
            if self.input.is_key_down('s'):
                self._lastkey = True
            else:
                self._lastkey = None
